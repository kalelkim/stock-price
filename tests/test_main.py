...
def test_bad_watchlist_propagates(tmp_path):
    from stock_price.watchlist import WatchlistError

    wl = tmp_path / "tickers.json"
    wl.write_text("{not valid json", encoding="utf-8")
    with pytest.raises(WatchlistError):
        run(
            watchlist_path=wl,
            data_dir=tmp_path / "data",
            now=datetime(2026, 5, 31, 8, 0, tzinfo=KST),
            reader=lambda s: None,
            sleep=lambda s: None,
        )
